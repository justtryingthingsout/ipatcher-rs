...
fn iboot_ver(buf: &[u8]) -> (usize, bool) {
...
fn get_rsa_patch(buf: &mut [u8], ver: &usize) {
...
fn get_debugenabled_patch(buf: &mut [u8]) {
...
fn get_bootargs_patch(buf: &mut [u8], args: &String) {
    ...
    let arg = match args.len() {
        ...
        val => panic!("Issue with program, value was {}", val) //should never happen, but Rust needs this wildcard anyways
    };
...
fn get_secrom_patch(buf: &mut [u8], ver: usize) {
    ...
    let beg_func2 = xref64(buf, 0, buf.len(), find2) - match ver {
        1940 => 0x44,
        2261 => 0x30,
        val => panic!("Issue with program, value was {}", val) //should never occur since the last match should've already panicked
    };
